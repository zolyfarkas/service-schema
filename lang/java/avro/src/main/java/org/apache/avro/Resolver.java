  ...
  public static class EnumAdjust extends Action {
    ...
    public static Action resolve(Schema w, Schema r, GenericData d) {
      ...
      for (int i = 0; i < adjustments.length; i++) {
        ...
        int j = r.getEnumSymbolOrAliasOrdinal(ws);
        if (j == -1) {
          // try writer aliasses
          Map<String, Set<String>> symbolAliases = w.getEnumSymbolAliases();
          if (symbolAliases != null) {
            Set<String> aliases = symbolAliases.get(ws);
            if (aliases != null) {
              for (String alias : aliases) {
                j = r.getEnumSymbolOrAliasOrdinal(alias);
                if (j != -1) {
                  break;
                }
              }
        ...
        if (j == -1) {
          String fallbackSymbol = r.getEnumDefault();
          if (fallbackSymbol != null) {
            adjustments[i] = r.getEnumOrdinal(fallbackSymbol);
          } else {
            adjustments[i] = -1;
          }
        } else {
